...
#### Candidate models #####
  ...
  res.top.model <- simulateResiduals(fittedModel = m.1) # change out for the model I am interested in
  plot(res.top.model)
...
################ Investigate number of offspring model ###################################
  ...
  ### make plot of first 2 ranking models compared
    ...
    # Now same for second ranking model
   ...
   ggplot(data = data2014_2023_adultfemales_noI63, aes(x = num.sons, y = mean.ep, color = age)) +
     geom_point(size = 2, alpha = 0.8) +
     xlab("Number of Alive Sons") +
     ylab("Eye Patch Ratio") +
     theme_classic(base_size = 16) +
   ...
   ggplot(data = data2014_2023_adultfemales_noI63, aes(x = num.daughters, y = mean.ep, color = age)) +
     geom_point(size = 2, alpha = 0.8) +
     xlab("Number of Alive Daughters") +
     ylab("Eye Patch Ratio") +
     theme_classic(base_size = 16) +
